...
std::string parseJobObject(const cups_job_t *job,
  ...
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("id"),
           V8_VALUE_NEW(Number, job->id));
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("name"),
           V8_STRING_NEW_UTF8(job->title));
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("printerName"),
           V8_STRING_NEW_UTF8(job->dest));
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("user"),
           V8_STRING_NEW_UTF8(job->user));
  ...
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("format"),
           V8_STRING_NEW_UTF8(job_format.c_str()));
  ...
  if (i_status == 0) {
    ...
    Nan::Set(result_printer_job_status, i_status++,
             V8_STRING_NEW_UTF8(s.str().c_str()));
  }
  ...
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("completedTime"),
           Nan::New<v8::Date>(completedTime).ToLocalChecked());
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("creationTime"),
           Nan::New<v8::Date>(creationTime).ToLocalChecked());
  Nan::Set(result_printer_job, V8_STRING_NEW_UTF8("processingTime"),
           Nan::New<v8::Date>(processingTime).ToLocalChecked());
 ...
 */
void populatePpdOptions(v8::Local<v8::Object> ppd_options, ppd_file_t *ppd,
                        ppd_group_t *group) {
  int i, j;
  ppd_option_t *option;
  ppd_choice_t *choice;
  ...
  for (i = group->num_options, option = group->options; i > 0; --i, ++option) {
    ...
    for (j = option->num_choices, choice = option->choices; j > 0;
         --j, ++choice) {
      Nan::Set(ppd_suboptions, V8_STRING_NEW_UTF8(choice->choice),
               V8_VALUE_NEW(Boolean, static_cast<bool>(choice->marked)));
    }
...
std::string parseDriverOptions(const cups_dest_t *printer,
                               v8::Local<v8::Object> ppd_options) {
  const char *filename;
  ppd_file_t *ppd;
  ppd_group_t *group;
  int i;
...
std::string parsePrinterInfo(const cups_dest_t *printer,
  ...
  Nan::Set(result_printer, V8_STRING_NEW_UTF8("name"),
           V8_STRING_NEW_UTF8(printer->name));
  Nan::Set(result_printer, V8_STRING_NEW_UTF8("isDefault"),
           V8_VALUE_NEW(Boolean, static_cast<bool>(printer->is_default)));

  if (printer->instance) {
    Nan::Set(result_printer, V8_STRING_NEW_UTF8("instance"),
             V8_STRING_NEW_UTF8(printer->instance));
  }
  ...
  for (int j = 0; j < printer->num_options; ++j, ++dest_option) {
    Nan::Set(result_printer_options, V8_STRING_NEW_UTF8(dest_option->name),
             V8_STRING_NEW_UTF8(dest_option->value));
  }
  Nan::Set(result_printer, V8_STRING_NEW_UTF8("options"),
           result_printer_options);
  // Get printer jobs
  cups_job_t *jobs;
...
public:
  ...
  CupsOptions(v8::Local<v8::Object> iV8Options) : num_options(0) {
    v8::Local<v8::Array> props =
        Nan::GetPropertyNames(iV8Options).ToLocalChecked();

    for (unsigned int i = 0; i < props->Length(); ++i) {
      v8::Local<v8::Value> key(Nan::Get(props, i).ToLocalChecked());
      Nan::Utf8String keyStr(V8_LOCAL_STRING_FROM_VALUE(key));
      Nan::Utf8String valStr(V8_LOCAL_STRING_FROM_VALUE(
          Nan::Get(iV8Options, key).ToLocalChecked()));
...
MY_NODE_MODULE_CALLBACK(getJob) {
  ...
  int totalJobs = cupsGetJobs(&jobs, *printername, 0 /*0 means all users*/,
                              CUPS_WHICHJOBS_ALL);
  if (totalJobs > 0) {
    int jobi = 0;
    cups_job_t *job = jobs;
...
MY_NODE_MODULE_CALLBACK(PrintDirect) {
  ...
  int job_id = cupsCreateJob(CUPS_HTTP_DEFAULT, *printername, *docname,
                             options.getNumOptions(), options.get());
  if (job_id == 0) {
    RETURN_EXCEPTION_STR(cupsLastErrorString());
  }

  if (HTTP_CONTINUE != cupsStartDocument(CUPS_HTTP_DEFAULT, *printername,
                                         job_id, *docname, type_str.c_str(),
                                         1 /*last document*/)) {
    RETURN_EXCEPTION_STR(cupsLastErrorString());
  }
...
MY_NODE_MODULE_CALLBACK(PrintFile) {
  ...
  int job_id = cupsPrintFile(*printer, *filename, *docname,
                             options.getNumOptions(), options.get());
